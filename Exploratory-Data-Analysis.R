...
####
with(airquality, plot(Wind,Ozone));abline(h=10,lwd=2,lty=2)
boxplot(Temp ~ Month, data=airquality, col="red")
barplot(table(airquality$Month,col="yellow",main("Data Points in each both")))
hist(airquality$Temp, col="gray", breaks = 100); abline(v=median(airquality$Temp), lwd=2, col="green")
abline(v=median(airquality$Temp), lwd=2)

##########
par(mfrow=c(1,2),mar=c(5,4,2,1));
with(subset(airquality, Month==5), plot(Temp, Wind), main = "May");abline(h=10,lwd=2,lty=2);
with(subset(airquality, Month==6), plot(Temp, Wind), main = "June");abline(h=10,lwd=2,lty=2)
